...
async function checkHealth(url: string): Promise<boolean> {
  if (!url) return false;
  // BEGIN-PANTHEON-OVERRIDE — in Pantheon mode, hit the OpenWork API /health endpoint
  // (the OpenCode SDK /global/health is workspace-scoped under Pantheon).
  if (isPantheonMode()) {
    try {
...
export function ServerProvider({ children, defaultUrl }: ServerProviderProps) {
  ...
  useEffect(() => {
    ...
    // BEGIN-PANTHEON-OVERRIDE — force Pantheon URL when running against Pantheon
    const forceProxy =
      isPantheonMode() ||
      (!isDesktopRuntime() &&
        isWebDeployment() &&
