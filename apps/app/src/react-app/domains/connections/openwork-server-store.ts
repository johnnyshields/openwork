...
export function createOpenworkServerStore(options: CreateOpenworkServerStoreOptions) {
  ...
  const getBaseUrl = () => {
    // BEGIN-PANTHEON-OVERRIDE — Pantheon serves the OpenWork API; ignore local host/server prefs
  ...
  const getAuth = () => {
    ...
    if (isPantheonMode()) {
      const jwt =
        (typeof window !== "undefined" ? window.localStorage.getItem("pantheon.jwt") : null)?.trim() ?? "";
  ...
  const syncFromOptions = () => {
    ...
    // BEGIN-PANTHEON-OVERRIDE — skip desktop port override in Pantheon mode
  ...
  const start = () => {
    ...
    const refreshHostInfo = () => {
      // BEGIN-PANTHEON-OVERRIDE — skip Tauri-only host info polling in Pantheon mode
      if (isPantheonMode()) return;
      // END-PANTHEON-OVERRIDE
      if (!isDesktopRuntime()) return;
      if (!options.documentVisible()) return;
