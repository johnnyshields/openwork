...
export const PANTHEON_LOGIN_URL = (process.env.NEXT_PUBLIC_PANTHEON_LOGIN_URL ?? "").trim();

/** Check URL for ?pantheon_token= param (redirect back from Pantheon login). */
export function extractPantheonTokenFromUrl(): string | null {
...
export function getWorkerSummary(payload: unknown): WorkerSummary | null {
  ...
  // Pantheon returns the worker flat; Den wrapped it in { worker, instance }.
  const worker = isRecord(payload.worker) ? payload.worker : payload;
  if (typeof worker.id !== "string" || typeof worker.name !== "string") {
    return null;
  }
